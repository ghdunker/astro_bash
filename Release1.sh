...
# TODO: Find a way to pass UTC to the date, right now it's breaking with "1971-03-29 09:00"
  Cur_M=$4
  start_date=$1
  end_date=$2
  sd_n=`date --date="$start_date 13:00" +%s` 
  ...
  do
    ...
    init_sign=$sign
    #LB
    if [[ $sign -eq $3 && $init_p -eq 0 && -z $LB ]];
    then
      LB="<--LB"
      sign=`Loose $sign`
      init_sign=$sign
    fi
    #echo -e ${Signs_S[$sign]} `date --date="$i_date" +%Y-%m-%d`  $LB
    date_from_S=`date --date="@${i_date}" +%Y-%m-%d\ %R`
    date_from_S_UT=`date --date="$date_from_S" +%Y-%m-%d\ %R`

    echo -e ${sign} `date --date="$date_from_S_UT" +%Y-%m-%d`
    #echo -e ${sign} `date --date="$date_from_S_UT" +%Y-%m-%d\ %R`
